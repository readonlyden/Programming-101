...
21. Написать программу, которая умеет менять значение двух целочисленных переменных между собой, не используя условные операторы, циклы, массивы, функции и так далее. Только простейшие операторы, только хардкор. И да, это вам не Python свой гонять с его `a, b = b, a` !
